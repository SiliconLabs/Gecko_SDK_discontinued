...
static SPRITE _aSprite[] = {
  { -90,  20, 10,  3, GUI_COUNTOF(_apImages_1), _apImages_1, 0, 0},
  { 290, 150, -8, -2, GUI_COUNTOF(_apImages_0), _apImages_0, 0, 0},
};
...
static void _Action(WM_HWIN hTree, int xSize, int ySize) {
    ...
    do {
      if (GUIDEMO_CheckCancel()) {
        ...
        if (GUIDEMO_GetConfFlag(GUIDEMO_CF_SHOW_SPRITES)) {
          int i;
          for (i = 0; i < (int)GUI_COUNTOF(_aSprite); i++) {
  ...
  } while (++Index < (int)GUI_COUNTOF(_aAction));
}
...
static void _DemoTreeview(void) {
  ...
  GUI_WIDGET_CREATE_INFO aCreate[] = {
    { FRAMEWIN_CreateIndirect,  "Explorer", 0,                0, 0, 0, 0 ,0 ,0 ,0},
    { TREEVIEW_CreateIndirect,  NULL,       GUI_ID_TREEVIEW0, 0, 0, 0, 0 ,0, 0, 0},
  };
