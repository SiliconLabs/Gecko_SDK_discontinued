...
static void HwFcManageClearToSend(uint8_t gpioPinNo)
{
  ...
  if (handle && handle->fcType == uartdrvFlowControlHw)
  {
    ...
    {
      // Only manage DMA if transfer is active and not already paused by SW
      bool active = false;
      Ecode_t status = DMADRV_TransferActive( handle->txDmaCh , &active );
      if ( (status == ECODE_EMDRV_DMADRV_OK) && active && handle->txDmaPaused == 0)
      {
...
#else
    handle->peripheral.leuart->ROUTE |= USART_ROUTE_RXPEN;
#endif
  }
}
                               ...
                               UARTDRV_Count_t count)
{
  ...
  if (handle->type == uartdrvUartTypeUart)
  {
    while (count--)
    {
      ...
      while (!(handle->peripheral.uart->STATUS & USART_STATUS_TXBL));
      handle->peripheral.uart->TXDATA = *data;
      data++;
    }
  ...
  else if (handle->type == uartdrvUartTypeLeuart)
  {
    ...
    {
      // Wait for space in TX buffer
      while (!(handle->peripheral.leuart->STATUS & LEUART_STATUS_TXBL));
      handle->peripheral.leuart->TXDATA = *data;
      data++;
    }
    // Wait for TX completion
...
@n @section uartdrv_api The API
  ...
  @ref UARTDRV_InitUart(), @ref UARTDRV_InitLeuart(), @ref UARTDRV_DeInit() @n
  ...
  @ref UARTDRV_FlowControlSet(), UARTDRV_FlowControlGetSelfStatus(), UARTDRV_FlowControlSetPeerStatus(), UARTDRV_FlowControlGetPeerStatus() @n
  ...
  @ref UARTDRV_PauseTransmit(), UARTDRV_ResumeTransmit() @n
...
@n @section uartdrv_fc Flow Control Support
  ...
  EMDRV_UARTDRV_HW_FLOW_CONTROL_ENABLE is set to 0. This reduces the code size
  and complexity of the driver.

  There are two types of flow control supported, hardware and software. To
  enable any of these, set EMDRV_UARTDRV_HW_FLOW_CONTROL_ENABLE to 1 in
...
@n @subsection uartdrv_fc_sw Software Flow Control
  ...
  control logic, then EMDRV_UARTDRV_HW_FLOW_CONTROL_ENABLE should be set to 0
