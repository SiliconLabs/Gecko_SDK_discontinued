...
bool mbedtls_ecp_device_grp_capable( const mbedtls_ecp_group *grp )
{
    switch( grp->id )
    {
        ...
        case MBEDTLS_ECP_DP_SECP192R1:
            ...
            CRYPTO_MulOperandWidthSet( cryptoMulOperandModulusBits );
...
#if defined( MBEDTLS_MPI_MUL_MPI_ALT ) || defined( MBEDTLS_MPI_MUL_INT_ALT )
            CRYPTO_MulOperandWidthSet( cryptoMulOperandModulusBits );
            CRYPTO_ResultWidthSet( cryptoResult260Bits );
#endif
            return( true );
...
#if defined(MBEDTLS_ECP_DP_SECP192R1_ENABLED)
        case MBEDTLS_ECP_DP_SECP192R1:
            CRYPTO_ModulusSet( cryptoModulusEccP192 );
            CRYPTO_MulOperandWidthSet( cryptoMulOperandModulusBits );
            CRYPTO_ResultWidthSet( cryptoResult256Bits );
            return 0;
#endif /* MBEDTLS_ECP_DP_SECP192R1_ENABLED */
...
#if defined(MBEDTLS_ECP_DP_SECP224R1_ENABLED)
        case MBEDTLS_ECP_DP_SECP224R1:
            CRYPTO_ModulusSet( cryptoModulusEccP224 );
            CRYPTO_MulOperandWidthSet( cryptoMulOperandModulusBits );
            CRYPTO_ResultWidthSet( cryptoResult256Bits );
            return 0;
#endif /* MBEDTLS_ECP_DP_SECP224R1_ENABLED */
...
#if defined(MBEDTLS_ECP_DP_SECP256R1_ENABLED)
        case MBEDTLS_ECP_DP_SECP256R1:
            CRYPTO_ModulusSet( cryptoModulusEccP256 );
            CRYPTO_MulOperandWidthSet( cryptoMulOperandModulusBits );
            CRYPTO_ResultWidthSet(cryptoResult260Bits);
            return 0;
#endif /* MBEDTLS_ECP_DP_SECP256R1_ENABLED */
 ...
 ******************************************************************************/
void mbedtls_ecp_device_deinit( const mbedtls_ecp_group *grp )
{
  (void) grp;
...
{
  uint32_t volatile* regPtr = (uint32_t volatile*) ddataReg;
  uint32_t* pVal = mpi->p;
                           ...
                           const mbedtls_ecp_point *P )
{
  ...
  */
  ...
  ecp_crypto_ddata_write(cryptoRegDDATA0, &P->Z);
  ecp_crypto_ddata_write(cryptoRegDDATA0, &P->Y);
  MBEDTLS_MPI_CHK( ecp_crypto_ddata_read(cryptoRegDDATA0, &R->Z) );
  ecp_crypto_ddata_write(cryptoRegDDATA4, &P->X);
  CRYPTO_DDataRead(cryptoRegDDATA0, _2YY);
  ...
  */
  ...
  MBEDTLS_MPI_CHK( ecp_crypto_ddata_read(cryptoRegDDATA0, &R->X) );
  ...
  */
  ...
  CRYPTO_DDataWrite(cryptoRegDDATA0, _2YY);
  MBEDTLS_MPI_CHK( ecp_crypto_ddata_read(cryptoRegDDATA0, &R->Y) );
                          ...
                          const mbedtls_ecp_point *P, const mbedtls_ecp_point *Q )
{
  ...
  */
  ...
  ecp_crypto_ddata_write(cryptoRegDDATA0, &P->Z);
  ecp_crypto_ddata_write(cryptoRegDDATA0, &Q->X);
  ecp_crypto_ddata_write(cryptoRegDDATA0, &Q->Y);
  ...
  */
  ...
  ecp_crypto_ddata_write(cryptoRegDDATA1, &P->Y);
  CRYPTO_EXECUTE_11(
                    CRYPTO_CMD_INSTR_MSUB,
  ...
  ecp_crypto_ddata_write(cryptoRegDDATA0, &P->X);
  MBEDTLS_MPI_CHK( ecp_crypto_ddata_read(cryptoRegDDATA0, &R->Z) );
                                ...
                                ecc_bigint_t  R)
{
    ...
    */
    ...
    CRYPTO_DDataWrite(cryptoRegDDATA1, Y);  /* Set C to Y (divisor) initially */
    CRYPTO_DDataWrite(cryptoRegDDATA2, X);  /* Set U to X (dividend)initially */
    
    CRYPTO_DDataWrite(cryptoRegDDATA3, N);  /* Set D to modulus p initially   */
    
    CRYPTO_EXECUTE_3(CRYPTO_CMD_INSTR_CLR,            /* DDATA0 = 0 */
    ...
    while ( !crypto_ddata0_is_zero(&status_reg) )
    {
        ...
        if ((lsb_C & 0x1) == 0)
        {
            CRYPTO_EXECUTE_3(CRYPTO_CMD_INSTR_SELDDATA1DDATA1,
        ...
        else
        {
            ...
            CRYPTO_EXECUTE_2(CRYPTO_CMD_INSTR_SELDDATA1DDATA3,
                             CRYPTO_CMD_INSTR_ADD);
            
            rdata = CRYPTO_DData0_4LSBitsRead();
            ...
            lsb_C = CRYPTO_DData0_4LSBitsRead();
            CRYPTO_EXECUTE_4(CRYPTO_CMD_INSTR_SELDDATA1DDATA1,
                             CRYPTO_CMD_INSTR_SHRA,
                             CRYPTO_CMD_INSTR_DDATA0TODDATA1,
                             CRYPTO_CMD_INSTR_DDATA3TODDATA0); /* to get the lsb of D(R3) */
            
            /*  R3 = D >> 1  */
            lsb_D = CRYPTO_DData0_4LSBitsRead();
            
            CRYPTO_EXECUTE_2(CRYPTO_CMD_INSTR_SELDDATA3DDATA3,
                             CRYPTO_CMD_INSTR_SHRA);
        ...
        lsb_U = CRYPTO_DData0_4LSBitsRead();

        /* if ((U[31:0] & 0x1) == 0x1) */
        if((lsb_U & 0x1) == 0x1)
        {
            CRYPTO_EXECUTE_10(CRYPTO_CMD_INSTR_SELDDATA2DDATA2,
                              CRYPTO_CMD_INSTR_SHRA,
                              CRYPTO_CMD_INSTR_DDATA0TODDATA2,
            ...
            CRYPTO_DDataWrite(cryptoRegDDATA0, N);
        }
        else
        {
            CRYPTO_EXECUTE_3(CRYPTO_CMD_INSTR_SELDDATA2DDATA2,
                             CRYPTO_CMD_INSTR_SHRA,
                             CRYPTO_CMD_INSTR_DDATA0TODDATA2);
        } 
    ...
    CRYPTO_EXECUTE_2(CRYPTO_CMD_INSTR_DDATA3TODDATA0,
                     CRYPTO_CMD_INSTR_DEC);
    
    if (crypto_ddata0_is_zero(&status_reg))
    {
        CRYPTO_DDataRead(cryptoRegDDATA4, R);
    }
    else
    {
        CRYPTO_DDataWrite(cryptoRegDDATA0, N);
        CRYPTO_EXECUTE_2(CRYPTO_CMD_INSTR_SELDDATA0DDATA4,
                         CRYPTO_CMD_INSTR_SUB
                         );
        CRYPTO_DDataRead(cryptoRegDDATA0, R);
    }
...
int ecp_device_normalize_jac( const mbedtls_ecp_group *grp, mbedtls_ecp_point *P )
{
    ...
    CRYPTO_DDataWrite(cryptoRegDDATA1, Z_inv);
    ...
    ecp_crypto_ddata_write(cryptoRegDDATA1, &Z_inv);
#endif
    ...
    */
    ...
    ecp_crypto_ddata_write(cryptoRegDDATA3, &P->X);
    ecp_crypto_ddata_write(cryptoRegDDATA4, &P->Y);
    
    CRYPTO_EXECUTE_12(CRYPTO_CMD_INSTR_DDATA1TODDATA2,
                      CRYPTO_CMD_INSTR_SELDDATA1DDATA2,
                      CRYPTO_CMD_INSTR_MMUL,
                      CRYPTO_CMD_INSTR_DDATA0TODDATA1,
                      CRYPTO_CMD_INSTR_SELDDATA1DDATA3,
    ...
    ecp_crypto_ddata_read(cryptoRegDDATA0, &P->Y);
    ecp_crypto_ddata_read(cryptoRegDDATA3, &P->X);
                                   ...
                                   mbedtls_ecp_point *T[], size_t t_len )
{
    ...
    for( i = 1; i < t_len; i++ )
    {
        ecp_crypto_ddata_write( cryptoRegDDATA1, &T[i]->Z );
        CRYPTO_DDataWrite( cryptoRegDDATA2, cc[i-1] );
        CRYPTO_EXECUTE_2(CRYPTO_CMD_INSTR_SELDDATA1DDATA2,
                         CRYPTO_CMD_INSTR_MMUL);
        CRYPTO_DDataRead( cryptoRegDDATA0, cc[i] );
    }
    ...
    for( i = t_len - 1; ; i-- )
    {
        ...
        {
            /* Z_inv (DDATA2) = uu */
            CRYPTO_DDataWrite(cryptoRegDDATA2, uu);
        }
        else
        {
            /* Z_inv (DDATA1) = uu x cc[i-1] modulo p */
            /* uu = uu x T[i]->Z modulo p */
            CRYPTO_DDataWrite(cryptoRegDDATA1, uu);
            CRYPTO_DDataWrite(cryptoRegDDATA2, cc[i-1]);
            ecp_crypto_ddata_write( cryptoRegDDATA3, &T[i]->Z );
            CRYPTO_EXECUTE_5(CRYPTO_CMD_INSTR_SELDDATA1DDATA2,
                             CRYPTO_CMD_INSTR_MMUL,
                             CRYPTO_CMD_INSTR_DDATA0TODDATA2, /* Z_inv (DDATA2) */
                             CRYPTO_CMD_INSTR_SELDDATA1DDATA3,
                             CRYPTO_CMD_INSTR_MMUL);
            CRYPTO_DDataRead(cryptoRegDDATA0, uu);
        }

        /*
         * proceed as in normalize()
         */
        ecp_crypto_ddata_write(cryptoRegDDATA3, &T[i]->X);
        ecp_crypto_ddata_write(cryptoRegDDATA4, &T[i]->Y);

        /* Z_inv  already in DDATA2 */
        CRYPTO_EXECUTE_12(CRYPTO_CMD_INSTR_DDATA2TODDATA1,
                          CRYPTO_CMD_INSTR_SELDDATA1DDATA2,
                          CRYPTO_CMD_INSTR_MMUL,
                          CRYPTO_CMD_INSTR_DDATA0TODDATA1,
                          CRYPTO_CMD_INSTR_SELDDATA1DDATA3,
        ...
        ecp_crypto_ddata_read(cryptoRegDDATA0, &T[i]->Y);
        ecp_crypto_ddata_read(cryptoRegDDATA3, &T[i]->X);
