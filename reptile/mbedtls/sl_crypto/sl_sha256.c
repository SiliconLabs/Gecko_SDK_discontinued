...
void mbedtls_sha256_process( mbedtls_sha256_context *ctx, const unsigned char data[64] )
{
    ...
    CRYPTO_EXECUTE_6( CRYPTO_CMD_INSTR_DMA0TODATA,
    ...
    CRYPTO_DDataWrite(cryptoRegDDATA0, ctx->state);

    /* Write block to QDATA1.  */
    CRYPTO_QDataWrite(cryptoRegQDATA1BIG, (uint32_t*) data);
    
    /* Read the state from crypto. */
    CRYPTO_DDataRead(cryptoRegDDATA0, ctx->state);
...
void mbedtls_sha256_finish( mbedtls_sha256_context *ctx, unsigned char output[32] )
{
    ...
    CRYPTO_CLOCK_ENABLE;
    CRYPTO_DDataRead(cryptoRegDDATA0BIG, (uint32_t*) output);
    CRYPTO_CLOCK_DISABLE;
