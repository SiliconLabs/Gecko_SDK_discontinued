/**************************************************************************//**
 * @file
 * @brief Weather station demo for SLSTK3401A and Sensors-EXP
 * @version 4.2.0
 ******************************************************************************
 * @section License
  ...
  while (1)
  {
    ...
    if (demoMode)
    {
      if (processGestures)
      {
        ...
        if (GPIO_PinInGet(gpioPortA, 1) == 0)
          handleGestures();
...
static void gpioSetup(void)
{
  ...
  /* Configure PA0 as pushpull. (5v enable signal) */
  GPIO_PinModeSet(gpioPortA, 0, gpioModePushPull, 0);
  GPIO_PinOutClear(gpioPortA, 0);

  /* Configure PA1 as input and enable interrupt - proximity interrupt. */
...
void GPIO_Unified_IRQ(void)
{
  ...
  if (interruptMask & (1 << 1))
  {
    /* Interrupt from Si1147 on PA1 */
