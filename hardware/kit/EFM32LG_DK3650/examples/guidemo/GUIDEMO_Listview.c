...
static int _AddRow(WM_HWIN hItem, int Insert) {
  ...
  //
  if (Insert) {
    r = LISTVIEW_InsertRow(hItem, 0, (const char* const*)pText);
  } else {
    r = LISTVIEW_AddRow(hItem, (const char* const*)pText);
  }
  return r;
}
...
static void _Action(WM_HWIN hList) {
  ...
  } while ((++Index < (int)GUI_COUNTOF(_aAction)) && (GUIDEMO_CheckCancel() == 0));
}
