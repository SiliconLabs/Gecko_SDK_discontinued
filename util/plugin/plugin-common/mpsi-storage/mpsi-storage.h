...
#define mpsiStorageRead(tag, flags, len, value) store_read(tag, flags, len, value)
#define mpsiStorageWrite(tag, flags, len, value) store_write(tag, flags, len, value)

#elif defined(EMBER_STACK_BLE)
#define mpsiStoragePrintln(...) do { printf(__VA_ARGS__); printf("\n"); } while(0)
