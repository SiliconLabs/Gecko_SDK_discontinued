...
static uint16_t FormatVersion = 0x0003;
...
static void vTraceStoreWarnings(void);
...
/* Store the current warnings */
static void vTraceStoreWarnings()
{
	TRACE_ALLOC_CRITICAL_SECTION();
	...
	if (RecorderEnabled)
	{
		...
		{
			vTracePrintF(trcWarningChannel, "TRC_SYMBOL_TABLE_SLOTS too small. Add %d slots.", NoRoomForSymbol);
		}
		...
		if (LongestSymbolName > 0)
		{
			...
			{
				vTracePrintF(trcWarningChannel, "TRC_SYMBOL_MAX_LENGTH too small. Add %d chars.", LongestSymbolName - (TRC_SYMBOL_MAX_LENGTH));
			}
		}
		...
		if (NoRoomForObjectData > 0)
		{
			...
			vTracePrintF(trcWarningChannel, "TRC_OBJECT_DATA_SLOTS too small. Add more slots.");
		}
...
#if (TRC_RECORDER_HARDWARE_PORT == TRC_PORT_ARM_Cortex_M_DWT)
void prvTraceInitCortexM()
{
	...
	do
	{
		...
		if (REG_DEMCR == 0)
		{
			psfError(PSF_ERROR_DWT_NOT_SUPPORTED);
			break;
		}
		...
		{
			psfError(PSF_ERROR_DWT_CYCCNT_NOT_SUPPORTED);
			break;
		}
...
static uint32_t prvGetTimestamp32(void)
{
#if ((HWTC_TYPE == TRC_FREE_RUNNING_32BIT_INCR) || (HWTC_TYPE == TRC_FREE_RUNNING_32BIT_DECR))
	return HWTC_COUNT;
#else /* ((HWTC_TYPE == TRC_FREE_RUNNING_32BIT_INCR) || (HWTC_TYPE == TRC_FREE_RUNNING_32BIT_DECR)) */
	int ticks = TRACE_GET_OS_TICKS();
	return (HWTC_COUNT & 0x00FFFFFF) + ((ticks & 0x000000FF) << 24);
#endif /* ((HWTC_TYPE == TRC_FREE_RUNNING_32BIT_INCR) || (HWTC_TYPE == TRC_FREE_RUNNING_32BIT_DECR)) */
}
