...
#define  OS_TRACE_ISR_REGISTER(isr_id, isr_name, isr_prio)  traceISR_REGISTER(isr_id, isr_prio)
...
#define  OS_TRACE_TASK_MSG_Q_CREATE(p_msg_q, p_name)        traceTASK_MSG_Q_CREATE(p_msg_q, p_name)
#define  OS_TRACE_TASK_MSG_Q_POST(p_msg_q)                  traceTASK_MSG_Q_SEND(p_msg_q)
#define  OS_TRACE_TASK_MSG_Q_POST_FAILED(p_msg_q)           traceTASK_MSG_Q_SEND_FAILED(p_msg_q)
#define  OS_TRACE_TASK_MSG_Q_PEND(p_msg_q)                  traceTASK_MSG_Q_RECEIVE(p_msg_q)
#define  OS_TRACE_TASK_MSG_Q_PEND_FAILED(p_msg_q)           traceTASK_MSG_Q_RECEIVE_FAILED(p_msg_q)
#define  OS_TRACE_TASK_MSG_Q_PEND_BLOCK(p_msg_q)            traceTASK_MSG_Q_RECEIVE_BLOCK(p_msg_q)

#define  OS_TRACE_TASK_SEM_CREATE(p_tcb, p_name)            traceTASK_SEM_CREATE(p_tcb, p_name)
#define  OS_TRACE_TASK_SEM_POST(p_tcb)                      traceTASK_SEM_SEND(p_tcb)
#define  OS_TRACE_TASK_SEM_POST_FAILED(p_tcb)               traceTASK_SEM_SEND_FAILED(p_tcb)
#define  OS_TRACE_TASK_SEM_PEND(p_tcb)                      traceTASK_SEM_RECEIVE(p_tcb)
#define  OS_TRACE_TASK_SEM_PEND_FAILED(p_tcb)               traceTASK_SEM_RECEIVE_FAILED(p_tcb)
#define  OS_TRACE_TASK_SEM_PEND_BLOCK(p_tcb)                traceTASK_SEM_RECEIVE_BLOCK(p_tcb)

#define  OS_TRACE_MUTEX_CREATE(p_mutex, p_name)             traceMUTEX_CREATE(p_mutex, p_name)
#define  OS_TRACE_MUTEX_DEL(p_mutex)                        traceMUTEX_DELETE(p_mutex)
#define  OS_TRACE_MUTEX_POST(p_mutex)                       traceMUTEX_SEND(p_mutex)
#define  OS_TRACE_MUTEX_POST_FAILED(p_mutex)                traceMUTEX_SEND_FAILED(p_mutex)
#define  OS_TRACE_MUTEX_PEND(p_mutex)                       traceMUTEX_RECEIVE(p_mutex)
#define  OS_TRACE_MUTEX_PEND_FAILED(p_mutex)                traceMUTEX_RECEIVE_FAILED(p_mutex)
#define  OS_TRACE_MUTEX_PEND_BLOCK(p_mutex)                 traceMUTEX_RECEIVE_BLOCK(p_mutex)
...
#define  OS_TRACE_SEM_CREATE(p_sem, p_name)                 traceSEM_CREATE(p_sem, p_name)
#define  OS_TRACE_SEM_DEL(p_sem)                            traceSEM_DELETE(p_sem)
#define  OS_TRACE_SEM_POST(p_sem)                           traceSEM_SEND(p_sem)
#define  OS_TRACE_SEM_POST_FAILED(p_sem)                    traceSEM_SEND_FAILED(p_sem)
#define  OS_TRACE_SEM_PEND(p_sem)                           traceSEM_RECEIVE(p_sem)
#define  OS_TRACE_SEM_PEND_FAILED(p_sem)                    traceSEM_RECEIVE_FAILED(p_sem)
#define  OS_TRACE_SEM_PEND_BLOCK(p_sem)                     traceSEM_RECEIVE_BLOCK(p_sem)

#define  OS_TRACE_Q_CREATE(p_q, p_name)                     traceQ_CREATE(p_q, p_name)
#define  OS_TRACE_Q_DEL(p_q)                                traceQ_DELETE(p_q)
#define  OS_TRACE_Q_POST(p_q)                               traceQ_SEND(p_q)
#define  OS_TRACE_Q_POST_FAILED(p_q)                        traceQ_SEND_FAILED(p_q)
#define  OS_TRACE_Q_PEND(p_q)                               traceQ_RECEIVE(p_q)
#define  OS_TRACE_Q_PEND_FAILED(p_q)                        traceQ_RECEIVE_FAILED(p_q)
#define  OS_TRACE_Q_PEND_BLOCK(p_q)                         traceQ_RECEIVE_BLOCK(p_q)

#define  OS_TRACE_FLAG_CREATE(p_grp, p_name)                traceFLAG_CREATE(p_grp, p_name)
#define  OS_TRACE_FLAG_DEL(p_grp)                           traceFLAG_DELETE(p_grp)
#define  OS_TRACE_FLAG_POST(p_grp)                          traceFLAG_SEND(p_grp)
#define  OS_TRACE_FLAG_POST_FAILED(p_grp)                   traceFLAG_SEND_FAILED(p_grp)
#define  OS_TRACE_FLAG_PEND(p_grp)                          traceFLAG_RECEIVE(p_grp)
#define  OS_TRACE_FLAG_PEND_FAILED(p_grp)                   traceFLAG_RECEIVE_FAILED(p_grp)
#define  OS_TRACE_FLAG_PEND_BLOCK(p_grp)                    traceFLAG_RECEIVE_BLOCK(p_grp)

#define  OS_TRACE_MEM_CREATE(p_mem, p_name)                 traceMEM_CREATE(p_mem, p_name)
#define  OS_TRACE_MEM_PUT(p_mem)                            traceMEM_SEND(p_mem)
#define  OS_TRACE_MEM_PUT_FAILED(p_mem)                     traceMEM_SEND_FAILED(p_mem)
#define  OS_TRACE_MEM_GET(p_mem)                            traceMEM_RECEIVE(p_mem)
#define  OS_TRACE_MEM_GET_FAILED(p_mem)                     traceMEM_RECEIVE_FAILED(p_mem)
