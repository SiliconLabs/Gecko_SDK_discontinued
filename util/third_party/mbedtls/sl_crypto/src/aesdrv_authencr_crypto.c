...
static inline int aesdrv_CMAC_Finalize( AESDRV_Context_t* pAesdrvContext,
                                            bool              encrypt,
                                            uint8_t*          digest,
...
/** Sequencer code for CCM authentication and encryption. There are 2 variants
 * of the code: MCU and DMA.
 */
 ...
 * 2 variants of the code: MCU and DMA.
 */
 ...
 * There are 2 variants of the code: MCU and DMA.
 */
...
/***************************************************************************//**
 * @brief
 *   Function is loading DATA0 register with provided data and result read
 *   back from DATA0 and input data is overwritten. Additionally,
 *   function handles zeropadding for last block. Function is supposed to be
 *   used for payload part of CCM where data is encrypted.
 *
 * @param[in] pData
 *   Input data
 *
 ...
 *
 ******************************************************************************/
static void aesdrvDataLoadStoreBle(CRYPTO_TypeDef *crypto,
                                   uint8_t        *pData,
                                   uint16_t        length)
{
  uint32_t tempBuf[4];
                  ...
                  const bool          encrypt)
{
  ...
  if (encrypt)
  {
    ...
    aesdrvDataLoadStoreBle( crypto, pData, dataLength );
 ...
 *
 ******************************************************************************/
static inline int aesdrv_CCM_Execute
(
 ...
 *
 * @details
 *  Function is a common implementation for GCM with MCU and DMA.
 *
 * @param pKey
 *  Key (128bit)
 ...
 *
 * @details
 *  Function is common for all data I/O modes (MCU, DMA).
 *
